...
@section('content')
    <div class="container">
        ...
        @if(Auth::user()->rol->description == 'administrator')
            <a class="btn btn-primary" href="{{ url('client/domicile/' . $detail->domicileSale->client->id)}}" style="float: right;">Regresar</a>
        @elseif(Auth::user()->rol->description == 'client')
            <a class="btn btn-primary" href="{{ route('activityHistory') }}" style="float: right;">Regresar</a>
        @else
            <a class="btn btn-primary" href="{{ route('employee/domiciles') }}" style="float: right;">Regresar</a>
        @endif
